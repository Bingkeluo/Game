...
type Connection struct {
	...
	//定义另外一个chan来控制关闭写端
	exitchan chan bool
	//断开连接时删除连接，需要再添加server，可以从server调用
	server iface.IServer
}

//新建一个通道，将从server中客户端获取的信息，在这里通过路由进行赋值，返回一个结构体，通过结构体的绑定具体的方法实现，
func NewConnection(conn *net.TCPConn,cid uint32,msghandler iface.IMsgHanler,server iface.IServer) iface.IConnection{
	return &Connection{
		conn:  conn,
		connID:  cid,
		isClosed: false,
		...
		server:server,//在创建连接时server将自己传递给conn
...
func (c *Connection)StartWriter(){
	fmt.Println("StartWriter Start...")
	defer fmt.Println("StartWriter Exit")

	for{
		select {
		case sendData:=<-c.msgchan:
...
func (c *Connection)StartReader(){
	...
	defer c.Stop()
		...
		for {
			...
			if config.GlobalConfig.WorkerSize>0 {
				c.msghandler.SendReqToQueue(req)
			}else {
...
func (c *Connection)Stop(){
	...
	//在Client停止时关闭连接
	c.server.GetConnMAgr().Remove(c.connID)
